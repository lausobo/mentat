...
impl StdError for SchemaConstraintViolation {}
...
impl DbError {
    ...
    pub fn with_source<E>(kind: DbErrorKind, source: E) -> DbError
    where
...
impl From<rusqlite::Error> for DbError {
    fn from(error: rusqlite::Error) -> DbError {
        ...
        DbError::with_source(DbErrorKind::RusqliteError(message), error)
    }
}
...
pub enum DbErrorKind {
    ...
    #[error("Cannot transact a fulltext assertion with a typed value that is not :db/valueType :db.type/string")]
...
{
    fn context(self, kind: DbErrorKind) -> Result<T> {
        self.map_err(|err| DbError::with_source(kind, err))
    }
}
